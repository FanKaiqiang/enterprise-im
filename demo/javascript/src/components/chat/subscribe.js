        ...
        for (var i in this.props.data) {
            if (!this.props.data[i].handled) {
                ...
                requests.push(
                    <li id={i} key={i}>
                        <span>{msg}</span>
                        <Button text={"同意"} onClick={this.agree} className='webim-subscribe-button'/>
                        <Button text={"拒绝"} onClick={this.reject} className='error webim-subscribe-button'/>
                    </li>
                );
            }
        }
...
module.exports = {
    ...
    show: function (data) {
        if (!this.requests[data.from]) {
            this.requests[data.from] = {msg: data.status, handled: false};
        } else {
