...
module.exports = React.createClass({
    ...
    getInitialState: function () {
        ...
        Demo.conn.listen({
            ...
            onOffline: function () {
                if (WebIM.config.isWindowSDK) {
                    Demo.api.NotifyError("网络连接中断，重新连接中...");
                } else {
                    //webRTC:断线处理
            ...
            onDeliveredMessage: function (message) {//发送已送达回执
                // TODO: Window SDK
                if (WebIM.config.isWindowSDK) {
            ...
            onReadMessage: function (message) {//发送已读回执
                // TODO: Window SDK
                if (WebIM.config.isWindowSDK) {
            ...
            onCreateGroup: function (message) {
                Demo.api.NotifySuccess('工作群组创建成功, 群组id: ' + message.data.groupid);
                me.getGroup();
            },
    ...
    handlePresence: function (msg) {
        ...
        switch (msg.type) {
            ...
            case 'deleteGroupChat':// The chat room or group is deleted.
                ...
                break;
            case 'addAdmin':
                Demo.api.NotifySuccess(msg.owner + '将您设为了会议' + msg.gid + '的管理员');
                break;
            case 'removeAdmin':
                Demo.api.NotifyError(msg.owner + '取消了您在' + msg.gid + '的管理员');
                break;
            case 'addMute':
                Demo.api.NotifyError(msg.owner + '将您在会议' + msg.gid + '中禁言');
                break;
            case 'removeMute':
